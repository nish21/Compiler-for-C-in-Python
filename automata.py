"""
Finite state automata for all possible keywords and operators
that we will be dealing with. 
Defined as '<keyword>_auto' for ASCII character set.
Returns the transition table. 
"""
def main_auto(charset = 256):
	table = []
	for i in range(6):
...
def reational_auto(charset = 256):
	table = []
	for i in range(10):
		table.append(list([-1]*charset))
	table[0][ord('<')] = 1
...
keywords = [main_auto, while_auto, break_auto, return_auto, const_auto, continue_auto]
data_types = [int_auto, float_auto, double_auto, char_auto, long_auto, short_auto]
ids = [id_auto, num_auto, stringliterals_auto]
ops = [logical_auto, reational_auto, bitwise_auto, arithmetic_auto]
